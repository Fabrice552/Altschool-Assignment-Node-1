...
function parseJSONBody(req) {
  return new Promise((resolve, reject) => {
    ...
    req.on('data', chunk => {
      data += chunk.toString();
      // guard for too large bodies (simple)
      if (data.length > 1e6) {
        req.socket.destroy();
...
async function handleDelete(req, res, id) {
  ...
  sendJSON(res, 200, { success: true, data: { id }, error: null });
}
...
exports.handle = async function (req, res) {
  ...
  const parts = parsed.pathname.replace(/^\/+|\/+$/g, '').split('/'); // remove leading/trailing slashes
